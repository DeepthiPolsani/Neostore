...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'D:/NeoStore/Final/Frontend/public/images')
...
router.post("/sociallogin", (req, res) => {
    ...
    Users.findOne({ email: req.body.email }).exec((err, data) => {
        if (err) {
            ...
            let ins = new Users({ name: req.body.name, lname: req.body.lname, email: req.body.email, password: "deepthi@1" ,phone:9999888877,gender:"female"});
            ins.save((err) => {
...
router.post("/addaddress" , (req, res) => {
    ...
    router.post("/editaddress", (req, res) => {
        ...
        console.log(req.body)
        Users.updateMany({},{$set:{"Address.$[elem].address":req.body.address,"Address.$[elem].pincode":req.body.pincode,"Address.$[elem].city":req.body.city,"Address.$[elem].state":req.body.state,"Address.$[elem].country":req.body.country}},{arrayFilters:[{"elem.Address_id":req.body.Address_id}]},(err,data)=>{
            if(err){
                        console.log(err);
...
router.put("/changepass/:id",async(req,res)=>{
    ...
    Users.updateOne({_id:id},{$set:{password:hashpassword,confirmpassword:confirmpassword}},(err)=>{
        if(err) {
            res.json({err:err})
        }
        else{
...
router.put("/changepassword/:id",autenticateToken,async(req,res)=>{
    ...
    Users.updateOne({_id:id},{$set:{fname:fname,lname:lname, email:email, password:hashpassword,
        confirmpassword:confirmpassword,phone:phone,gender:gender}},(err)=>{
        if(err) res.json({err:err});
